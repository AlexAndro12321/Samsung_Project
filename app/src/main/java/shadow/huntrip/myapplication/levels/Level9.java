...
public class Level9 extends AppCompatActivity
{
    ...
    private void InitializeExplainDialog()
    {
        ...
        previewImg.setImageResource(R.drawable.nirvana_logo);

        TextView textView = _explainDialog.findViewById(R.id.dialogText);
        textView.setText(R.string.Nirvana_text);
    }
    ...
    private void GetSongData()
    {
        try
        {
            _tempSounds = new ArrayList<>(Arrays.asList(getAssets().list("Level9_music")));

            for(int i = 0; i < _tempSounds.size(); i++)
                _tempSounds.set(i, "Level9_music/" + _tempSounds.get(i));
        }
        catch (IOException e)
        {
            e.printStackTrace();
    ...
    private void SelectRandomSong()
    {
        ...
        catch (Exception e)
        {
            if(_correctAnswersCounter >= 8 && _correctAnswersCounter <= 10)
            {
                ...
                CustomDialog.ShowPositiveDialog(this, Level10.class, this, _correctAnswersCounter);
            }
            else
            {
                switch(_correctAnswersCounter)
                {
                    case 0, 5, 6, 7 -> CustomDialog.ShowNegativeDialog(this, Level9.class, this, _correctAnswersCounter, "баллов");
                    case 1 -> CustomDialog.ShowNegativeDialog(this, Level9.class, this, _correctAnswersCounter, "балл");
                    case 2, 3, 4 -> CustomDialog.ShowNegativeDialog(this, Level9.class, this, _correctAnswersCounter, "балла");
